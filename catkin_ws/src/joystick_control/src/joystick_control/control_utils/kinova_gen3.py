    ...
    def __init__(
        ...
        try:
            ...
            self.joint_names = rospy.get_param(
                "/{}/kortex_driver/joint_names".format(self.robot_name)
            )
            self.is_gripper_present = rospy.get_param(
                "/{}/is_gripper_present".format(self.robot_name), True
            )
            ...
            self.action_topic_sub = rospy.Subscriber(
                "/{}/action_topic".format(self.robot_name),
                ActionNotification,
                self._action_topic_cb,
            )
            ...
            self.joint_state_sub = rospy.Subscriber(
                "/{}/base_feedback/joint_state".format(self.robot_name),
                JointState,
                self._joint_state_cb,
            )

            self.base_feedback_sub = rospy.Subscriber(
                "/{}/base_feedback".format(self.robot_name),
                BaseCyclic_Feedback,
            ...
            rospy.logerr(
                "Failed to initialize Kinova Gen3, {}!".format(self.robot_name)
            )
    ...
    def _init_gen3_services(self):
        ...
        try:
            ...
            set_cartesian_reference_frame_full_name = (
                "/{}/control_config/set_cartesian_reference_frame".format(
                    self.robot_name
                )
            )
            ...
            # Gripper command
            send_gripper_command_full_name = "/{}/base/send_gripper_command".format(
                self.robot_name
            )
            ...
            activate_publishing_of_action_notification_full_name = (
                "/{}/base/activate_publishing_of_action_topic".format(self.robot_name)
            )
            ...
            get_product_configuration_full_name = (
                "/{}/base/get_product_configuration".format(self.robot_name)
            )
            ...
            validate_waypoint_list_full_name = "/{}/base/validate_waypoint_list".format(
                self.robot_name
            )
            ...
            get_cartesian_full_name = "/{}/base/get_measured_cartesian_pose".format(
                self.robot_name
            )
            rospy.wait_for_service(get_cartesian_full_name)
            self.get_cartesian_pose = rospy.ServiceProxy(
                get_cartesian_full_name, GetMeasuredCartesianPose
            )
        ...
        for i in range(n[0], n[1] + 1):
            ...
            M_new = np.array(
                [
                    [
                        np.cos(theta),
                        -np.cos(DH_PARAMETERS[i][0]) * np.sin(theta),
                        np.sin(DH_PARAMETERS[i][0]) * np.sin(theta),
                        DH_PARAMETERS[i][1] * np.cos(theta),
                    ],
                    [
                        np.sin(theta),
                        np.cos(DH_PARAMETERS[i][0]) * np.cos(theta),
                        -np.sin(DH_PARAMETERS[i][0]) * np.cos(theta),
                        DH_PARAMETERS[i][1] * np.sin(theta),
                    ],
                    [
                        0,
                        np.sin(DH_PARAMETERS[i][0]),
                        np.cos(DH_PARAMETERS[i][0]),
                        DH_PARAMETERS[i][2],
                    ],
                    [0, 0, 0, 1],
                ]
    ...
    def dh_mat_inv(self, n):
        ...
        R_T = np.array(
            [
                ...
                [
                    -np.cos(DH_PARAMETERS[n][0]) * np.sin(theta),
                    np.cos(DH_PARAMETERS[n][0]) * np.cos(theta),
                    np.sin(DH_PARAMETERS[n][0]),
                ],
                [
                    np.sin(DH_PARAMETERS[n][0]) * np.sin(theta),
                    -np.sin(DH_PARAMETERS[n][0]) * np.cos(theta),
                    np.cos(DH_PARAMETERS[n][0]),
                ],
            ]
        )
...
class RGBDVision(object):
    ...
    def _image_callback(
        ...
        if self.depth is not None:
            cv2.imshow("{} Depth".format(self.name), np.uint8(self.depth))
        key = cv2.waitKey(1)
