...
class png_control(KinovaGen3):
    def __init__(self, args):
        super(png_control, self).__init__(args)
        self.run = True  # prevents robot from stepping
        self.ee_flat = np.zeros(
            7
        )  # placeholder vector to operate last joint to keep it flat
        self.lengths = np.array(
            [156.4, 128.4, 210.4, 210.4, 208.4, 105.9, 105.9, 61.5, 140]
        )  # lengths of robot joints, could replace with dh-params
    ...
    def remap_axes(self, vec, mode):
        ...
        if mode == 0:  # CARTESIAN
            twist = self.threshold(
                vec[5], 0.5
            )  # thresholding third joystick dof, twist is vec[2] but currently mapped to vec[5] (thumb button)
        else:
            twist = self.threshold(vec[4], 0.5)
        control_vec = np.array(
            [[1, vec[1], vec[0], twist]]
        )  # defining control vec, can change indicies of vec to use different dofs of joystick
        self.state_to_qdot(mode)  # updating qdots
        if mode == 1:  # JOINT CONTROL
            self.j6_theta_change(
                -twist
            )  # this is to change the parameter of the pid controller operating the last joint
        # finding the final actions sent to the robot
        cmd = np.dot(
            control_vec,
            np.array([self.ee_flat, self.fb_qdot, self.lr_qdot, self.tw_qdot]),
        )[0].tolist()
    ...
    def state_to_qdot(self, mode):
        ...
        ee_z = (
            self.get_ee_z_frame()
        )  # getting end effector coordinate frame in base coordinates

        # translation mode
        if mode == 0:
            fb_basis = np.array(
                [ee_z[0][0], ee_z[1][0], 0, 0, 0, 0, 0, 0]
            )  # translations defined by first dof of joystick
            lr_basis = np.array(
                [0, 0, 0, 0, 0, 0, -np.cos(self.j6_angle), -np.sin(self.j6_angle)]
            )  # translations defined by second dof of joystick
            tw_basis = np.array(
                [0, 0, 1, 0, 0, 0, 0, 0]
            )  # translations defined by third dof of joystick
            self.fb_qdot, self.lr_qdot, self.tw_qdot = self.parse_bases(
                fb_basis, lr_basis, tw_basis
            )  # getting qdots

        # rotation mode
        if mode == 1:
            fb_basis = np.array(
                [0, 0, 0, -np.sin(self.j6_angle), np.cos(self.j6_angle), 0, 0, 0]
            )
            lr_basis = np.array(
                [0, 0, 0, -np.cos(self.j6_angle), -np.sin(self.j6_angle), 0, 0, 0]
            )
    ...
    def get_ee_z_frame(self):
        ...
        delta_z = (T_base_ee @ (ref_x_ee.T @ rot_z(self.j6_angle)).T)[
            2
        ] - ee_origin_base[2]
    ...
    def check_cmd(self, command):
        ...
        for i in range(7):
            if (
                future_joint_pos[i] < JOINT_LIMIT[i][0]
                or future_joint_pos[i] > JOINT_LIMIT[i][1]
            ):
    ...
    def rotate_qdot(self, basis):
        ...
        v_lat = np.array(
            [[-W_lat[0][3] * 1000], [-W_lat[1][3] * 1000], [-W_lat[2][3] * 1000], [1]]
        )  # finding excess translations
        ...
        lat_vector = np.array(
            [
                ...
                0,
                base_vels_lat[2][0],
                lat_joint_vel_2D[0] + lat_joint_vel_2D_2[0],
                lat_joint_vel_2D[1] + lat_joint_vel_2D_2[1],
                -W_lat[0][1],
            ]
        )
    ...
    def jacob_base_inv(self):
        ...
        jacob_inv = np.linalg.inv(
            np.array(
                [[dxdt0, dxdt1, dxdt3], [dydt0, dydt1, dydt3], [dzdt0, dzdt1, dzdt3]]
            )
        )
    ...
    def j6_control(self, in_p):
        ...
        p = in_p[0]  # NOTE: removed negative sign so camera stays upside down.
        kp = 3
        kd = 1
...
# class pngng_control(KinovaGen3):
#     def __init__(self, args):
#         super(pngng_control, self).__init__(args)
#         self.run = True  # prevents robot from stepping
#         self.ee_flat = np.zeros(
#             7
#         )  # placeholder vector to operate last joint to keep it flat
...
#     def step(self, ax, mode, custom_commands):
#         # custom_commands allow us to add automatic control on top of the joystick commands
#         # runs each loop according to rospy.rate
#         if self.run:
#             cmd_main = self.remap_axes(ax, mode)

#             cmds_custom = []
#             for custom_command in custom_commands:
#                 # add custom gains for what our motion
#                 old_trans_gain = self.translation_gain
#                 old_rot_gain = self.rotation_gain
#                 old_wrist_gain = self.wrist_motion_gain
#                 self.translation_gain = custom_command.trans_gain
#                 self.rotation_gain = custom_command.rot_gain
#                 self.wrist_motion_gain = custom_command.wrist_gain

#                 cmds_custom.append(
#                     self.remap_axes(custom_command.ax, custom_command.mode)
#                 )

#                 self.translation_gain = old_trans_gain
#                 self.rotation_gain = old_rot_gain
#                 self.wrist_motion_gain = old_wrist_gain

#             cmd_total = self.check_cmd(np.sum([cmd_main, *cmds_custom], axis=0))
#             self.send_joint_speeds_command(cmd_total)
...
class cartesian_control(KinovaGen3):
    ...
    def remap_axes(self, vec, mode):
        if self.DOF == 3:
            if mode == 0:
                # vec[5] for z is minijoy, could use?
...
class joint_control(KinovaGen3):
    def __init__(self, args):
        # global
        super(joint_control, self).__init__(args)
        self.run = True
...
class xbox_control(KinovaGen3):
    def __init__(self):
        # global 
        super(xbox_control, self).__init__()
        self.run = True
        self.prev_cv_cmd = np.zeros(6)
        self.joint_velocities = np.zeros(6)
        self.home_array = np.array([0.1, 65, -179.9, -120, 0, 100, 90])
    ...
    def step(self, ax, mode):
        if self.run:
            cmd_main = self.remap_axes(ax)

            # cmds_custom = []
            # for custom_command in custom_commands:
            #     cmds_custom.append(self.remap_axes(custom_command.ax))

            # cmd_total = np.sum([cmd_main, *cmds_custom], axis=0)
            # cmd_total = np.sum([cmd_main], axis=0)
            # self.joint_velocities = cmd_total
            self.send_cartesian_velocity(cmd_main)
